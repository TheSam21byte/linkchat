...
import { getMyServers, joinServer } from '../services/members-api'
import { getServers } from '../services/servers-api'
...
function AppHome({
  ...
  useEffect(() => {
    ...
    async function loadHome() {
      try {
        const loadedMemberships = await getMyServers()
        if (!isActive) return
  ...
  async function handleJoin(server) {
    try {
      ...
      setError('')
      const membership = await joinServer(server.id)
