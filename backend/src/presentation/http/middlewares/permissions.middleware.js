...
import Member from "../../../infrastructure/persistence/mongoose/models/Member.js";
import Channel from "../../../infrastructure/persistence/mongoose/models/Channel.js";
import Message from "../../../infrastructure/persistence/mongoose/models/Message.js";
import Invitation from "../../../infrastructure/persistence/mongoose/models/Invitation.js";
...
export async function getServerIdFromChannelId(channelId) {
  ...
  return channel?.serverId?.toString?.() ?? channel?.serverId ?? null;
}
...
export async function getServerIdFromMessageId(messageId) {
  ...
  return getServerIdFromChannelId(message.channelId);
}

export async function getServerIdFromInvitationCode(code) {
  const invitation = await Invitation.findOne({ code });
  return invitation?.serverId?.toString?.() ?? invitation?.serverId ?? null;
}
